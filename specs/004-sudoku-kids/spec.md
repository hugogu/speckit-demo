...
3. **Given** 打印的纸质游戏，**When** 孩子完成后想核对答案，**Then** [NEEDS CLARIFICATION: 是否需要提供二维码/编号来查询答案？还是打印时附带答案页？]
...
- **FR-016**: 界面 MUST 在320px-1920px宽度范围内自适应显示，所有交互元素最小44x44px
- **FR-017**: 系统 MUST 在无网络连接时支持已加载游戏的基本离线游玩（除打印外）
...
- **PrintJob（打印任务）**: 代表一次打印请求的配置，包含生成数量、难度、面板大小、是否附带答案页、页面尺寸（A4/Letter）
...
- [NEEDS CLARIFICATION: 是否需要用户账户系统来支持多设备同步游戏历史？还是仅本地存储？]
- [NEEDS CLARIFICATION: 打印功能是否需要附带答案页？还是以其他方式提供答案查询？]
